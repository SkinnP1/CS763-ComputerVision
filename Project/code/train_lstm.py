...
for e in range(EPOCHS):
	...
	if (e>0 and e%20 == 0) or e==EPOCHS-1:
		...
		if max_test_acc < test_acc:
			...
			torch.save(model.state_dict(),RESULTS_DIR_ROOT+args.feat+"_checkpoint_lstm_"+args.type+".p")
			print("Saved Checkpoint")

PlotLoss(train_loss_list,test_loss_list,args.feat+"_loss_lstm_"+args.type+".jpg")
PlotAccuracy(train_accuracy_trace,test_accuracy_trace,args.feat+"_acc_lstm_"+args.type+".jpg")
PlotConfusionMatrix(train_words_true,train_words_pred,np.arange(N_CLASS),args.feat+"_conf_mat_train_lstm_"+args.type+".jpg",title = "train confusion matrix")
PlotConfusionMatrix(test_words_true,test_words_pred,np.arange(N_CLASS),args.feat+"_conf_mat_test_lstm_"+args.type+".jpg",title = "test confusion matrix")